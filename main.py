                ...
                if event.type == pg.MOUSEBUTTONUP:
                    ...
                    if Game.clicked_on_board(mouse_pos, board.pos): #player clicked on board
                        ...
                        if board.clicked_piece: #piece is selected
                            valid_squares = valid_squares = Game.get_valid_moves(board.clicked_piece.square, board.position, board.moves)
                            ...
                            if piece_on_clicked_square:
                                ...
                                if Game.turn == 0: #white to move
                                    if piece_on_clicked_square.get_color() == Color.WHITE:
                                        valid_squares = Game.get_valid_moves(piece_on_clicked_square.square, board.position, board.moves)
                                    ...
                                    if piece_on_clicked_square.get_color() == Color.BLACK:
                                        valid_squares = valid_squares = Game.get_valid_moves(piece_on_clicked_square.square, board.position, board.moves)
